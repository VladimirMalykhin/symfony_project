...
namespace App\Service\TemplateService\AliService;
...
class AliService
{
    ...
    private int $statistic_params_flag = 0;
    private string $statistic_params = '?session_id=44444&user_id=555555';
    ...
    private function parse_img($item, $number, $template_name, $locale) :array
    {
        ...
        foreach($param_object->getUsedParams('media', $item['props']) as $param_name){
            if(isset($item['props'][$param_name]) && $item['props'][$param_name] != '') {
                ...
                $item['props'][$param_name] .= $this->statistic_params;
                $this->check_statistic_params();
        ...
        foreach($param_object->getUsedArrayParams('media_array', $item['props']) as $param_name){
            for ($i = 0; $i < count($item['props'][$param_name]); $i++){
                if(isset($item['props'][$param_name])){
                    ...
                    $item['props'][$param_name][$i] .= $this->statistic_params;
                    $this->check_statistic_params();
    ...
    private function check_statistic_params() :void
    {
        if ($this->statistic_params_flag === 0) {
            $this->statistic_params = '';
            $this->statistic_params_flag = 1;
        }
    }
    ...
    {
        $this->styles .= file_get_contents($this->collections_folder . '/Base/alistyle.css');
    ...
    {
        $this->styles .= file_get_contents($this->collections_folder . '/' . $content['component'] . '/Ali' . $content['component'] . '.css');
    }
